"""แหล่งความจริงของลำดับ Flow — ห้ามสลับ/ข้ามโดยไม่ยืนยันกับผู้ใช้

โฟล์แรก: ผู้ใช้เปิด Express อยู่หน้า Dialog เลือกข้อมูลแล้ว → AutoKey กด ค้นหา ให้เลย (ไม่กด 8)
โฟล์สุดท้าย: กด 8 → Dialog เลือกข้อมูล → ค้นหา → … (ขั้นค้นหาเหมือนโฟล์แรก)

ลำดับรวม:
  FLOW_1_START → FLOW_2 (PV ทุกแถว) → FLOW_1_END (กด 8)
"""
...
FULL_SEQUENCE_NOTE = (
    "โฟล์แรก (ค้นหาบริษัทในหน้าเลือกข้อมูล) → โฟล์ 2 (PV ทุกแถว) → โฟล์สุดท้าย (กด 8)"
)
