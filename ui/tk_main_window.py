...
class MainWindow:
    ...
    def _build_pnd30_page(self, page: ttk.Frame) -> None:
        ...
        ttk.Label(header, text=UI_TEXT["menu_pnd30"], font=("Tahoma", 12, "bold")).pack(side="left", padx=12)

        form = ttk.Frame(page)
        form.pack(fill="x", padx=20, pady=(16, 0))
        ttk.Label(form, text=UI_TEXT["pp30_pdf_folder"]).grid(row=0, column=0, sticky="w")
        ttk.Entry(form, textvariable=self.pnd30_pdf_folder, width=42).grid(row=0, column=1, sticky="ew", padx=(8, 8))
        ttk.Button(form, text=UI_TEXT["choose_folder"], command=self._choose_pnd30_folder).grid(row=0, column=2)
        ...
        progress_row = ttk.Frame(status)
        progress_row.pack(fill="x", padx=8, pady=(8, 4))
        self.pnd30_progress = ttk.Progressbar(progress_row, maximum=100)
        self.pnd30_progress.pack(side="left", fill="x", expand=True)
        ttk.Label(progress_row, textvariable=self.pnd30_progress_text, width=16).pack(side="left", padx=(8, 0))
        ttk.Button(status, text=UI_TEXT["copy_log"], command=self._copy_pnd30_log).pack(anchor="e", padx=8)
        log_row = ttk.Frame(status)
        log_row.pack(fill="both", expand=True, padx=8, pady=(0, 8))
        log_row.rowconfigure(0, weight=1)
        log_row.columnconfigure(0, weight=1)
        self.pnd30_log_box = tk.Text(log_row, height=10, wrap="word")
        scroll = ttk.Scrollbar(log_row, orient="vertical", command=self.pnd30_log_box.yview)
        self.pnd30_log_box.configure(yscrollcommand=scroll.set)
    ...
    def _load_pnd30_folder(self) -> None:
        folder = Path(self.pnd30_pdf_folder.get().strip()).expanduser()
    ...
    def _run_pnd30(self, form_config: Pnd30FormConfig, express_data_dir: Path) -> None:
        try:
            Pnd30MatchRunService.run(
                form_config,
                express_data_dir,
                on_status=lambda message: self.root.after(0, lambda m=message: self._append_pnd30_log(m)),
                on_progress=lambda done, total: self.root.after(
                    0, lambda d=done, t=total: self._set_pnd30_progress(d, t)
                ),
            )
        except ValueError as exc:
            self.root.after(0, lambda text=str(exc): messagebox.showwarning(UI_TEXT["app_title"], text))
            ...
            self.root.after(0, self._pnd30_finished)
    ...
    def _set_pnd30_progress(self, done: int, total: int) -> None:
        percent = 0 if total <= 0 else int(round(done * 100 / total))
        self.pnd30_progress["value"] = percent
        self.pnd30_progress_text.set(UI_TEXT["pp30_progress"].format(done=done, total=total, percent=percent))

    def _append_pnd30_log(self, message: str) -> None:
        self.pnd30_log_box.insert("end", message + "\n")
        self.pnd30_log_box.see("end")

    def _copy_pnd30_log(self) -> None:
        text = self.pnd30_log_box.get("1.0", "end-1c")
        if not text.strip():
            return
        self.root.clipboard_clear()
        self.root.clipboard_append(text)
