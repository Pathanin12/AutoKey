...
from models.pnd30_pdf_record import Pnd30PdfRecord
...
class Pnd30PdfService:
    ...
    @staticmethod
    def load_record(pdf_path: Path) -> Pnd30PdfRecord:
        text = Pnd30PdfService.read_text(pdf_path)
        name = Pnd30PdfService.extract_company_name(text)
        if not name:
            name = tidy_name(pdf_path.stem)
        return Pnd30PdfRecord(
            pdf_path=pdf_path,
            company_name=name,
            form_values=Pnd30PdfService.extract_form_values(text),
        )
